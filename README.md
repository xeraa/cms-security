...
* MODX is the only system, which did a complete rewrite from version one to two (I am not sure about ExpressionEngine, but could not find any final information on this topic). [As pointed out](https://github.com/xeraa/cms-security/issues/1), this is a disadvantage. However as stated before, all actively supported versions are counted and [MODX Evolution](http://modx.com/software/open-source-cms/modx-evolution/) seems to be pretty much alive. And it would be unfair to compare only one major version against two or even three supported releases from the other projects.
